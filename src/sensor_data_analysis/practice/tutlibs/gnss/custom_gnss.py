...
class GNSSProcessor:
    ...
    def _parse_gga(self, line: str) -> Optional[Dict]:
        """
        GGA 메시지를 파싱합니다.
        GGA: $GPGGA,utc,lat,lat_dir,lon,lon_dir,fix_quality,num_satellites,hdop,altitude,altitude_unit,undulation,u_units,age,stn_ID,checksum,[CR][LF]
        """
        try:
            parts = line.split(',')
            if len(parts) < 15:
            ...
            return {
                ...
                'latitude': latitude,
                'longitude': longitude,
                'fix_quality': int(parts[6]), # TODO: FIX 품질 추출
                'num_satellites': int(parts[7]), # TODO: 위성 수 추출
                'hdop': float(parts[8]), # TODO: HDOP 추출
                'altitude': float(parts[9]), # TODO: 고도 추출
                'altitude_unit': parts[10], # 고도 단위 추출
                'geoid_height': float(parts[11]) # 지구 고도 추출 (undulation)
            }
    ...
    def _parse_rmc(self, line: str) -> Optional[Dict]:
        """
        RMC 메시지를 파싱합니다.
        RMC: $GPRMC,utc, pos_status, lat, lat_dir,lon,lon_dir,speed_Knots,track_true,date,mag_var,var_dir,mode_ind,checksum,[CR][LF]
        """
        try:
            parts = line.split(',')
            if len(parts) < 12:
            ...
            return {
                ...
                'latitude': latitude,
                'longitude': longitude,
                'speed_knots': float(parts[7]), # 속도 추출
                'course': float(parts[8]), # 방향 추출 (Track true)
                'date': parts[9] # 날짜 추출
            }
        except (ValueError, IndexError):
            return None
    ...
    def wgs84_to_enu(self, llh: np.ndarray, ref_llh: np.ndarray) -> np.ndarray:
        ...
        for idx in range(llh.shape[0]):
            ...
            enu[idx, 2] = ref_height - llh[idx, 2] # TODO: Up 추출
    ...
    def create_satellite_map(self, latitudes: List[float], longitudes: List[float], 
        ...
        # 시작점과 끝점 마커
        folium.Marker(
            location=[latitudes[0], longitudes[0]], # TODO: 시작점 좌표 대입
            popup="Start Point",
            icon=folium.Icon(color='green', icon='play')
        ).add_to(m)
        
        folium.Marker(
            location=[latitudes[-1], longitudes[-1]], # TODO: 끝점 좌표 대입
            popup="End Point",
