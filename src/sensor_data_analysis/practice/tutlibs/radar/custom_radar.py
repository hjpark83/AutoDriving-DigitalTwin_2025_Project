...
class StatisticalOutlierRemoval:
    ...
    def compute_statistics(self, mean_distances: np.ndarray) -> Tuple[float, float, float, float]:
        ...
        threshold_lower = global_mean - self.std_ratio * global_std # TODO: self.std_ratio를 사용하여 하한 임계값 계산
...
def custom_translation(pcd: o3d.geometry.PointCloud, vector: np.ndarray) -> o3d.geometry.PointCloud:
    ...
    translated_xyz = xyz + vector[np.newaxis, :] # TODO: np.newaxis를 사용하여 vector를 xyz에 더하기
...
def custom_rotation(pcd: o3d.geometry.PointCloud, rotation_matrix: np.ndarray, center: np.ndarray = np.array([0, 0, 0])) -> o3d.geometry.PointCloud:
    ...
    rotation_xyz = np.matmul(rotation_matrix, (xyz - center).T).T # TODO: 회전 행렬과 center 좌표를 적용하여 xyz 좌표 회전 (np.asarray(pcd.points).shape 확인 필수)
    rotation_xyz = rotation_xyz + center # TODO: 회전 후 center 좌표를 다시 더하여 xyz 좌표 원위치
...
def custom_pcd_transformation(pcd: o3d.geometry.PointCloud, transformation_matrix: np.ndarray) -> o3d.geometry.PointCloud:
    ...
    """
    xyz = np.asarray(pcd.points)
    homogeneous_xyz = np.concatenate([xyz, np.ones((xyz.shape[0], 1))], axis=1) # TODO: (N, 4) 형태로 xyz 좌표에 1을 추가하여 동차 좌표로 변환
    transformation_xyz = np.matmul(transformation_matrix, homogeneous_xyz.T).T # TODO: transformation_matrix와 homogeneous_xyz를 곱하여 변환된 좌표 계산 (np.matmul 사용)
