...
class CameraSimulator:
    ...
    def world_to_camera_frame(self, world_x, world_y, world_z):
        ...
        R_z = np.array([
            [math.cos(camera_yaw_rad), -math.sin(camera_yaw_rad), 0.0],
            [math.sin(camera_yaw_rad),  math.cos(camera_yaw_rad), 0.0],
            [0.0, 0.0, 1.0]
        ])
        
        R_y = np.array([
            [math.cos(camera_pitch_rad), 0.0, math.sin(camera_pitch_rad)],
            [0.0, 1.0, 0.0],
            [-math.sin(camera_pitch_rad), 0.0, math.cos(camera_pitch_rad)]
        ])

        R_x = np.array([
            [1.0, 0.0, 0.0],
            [0.0, math.cos(camera_roll_rad), -math.sin(camera_roll_rad)],
            [0.0, math.sin(camera_roll_rad),  math.cos(camera_roll_rad)]
        ])    
        ...
        T_vehicle_to_camera[0, 3] = self.camera_offset_x
        T_vehicle_to_camera[1, 3] = self.camera_offset_y
        T_vehicle_to_camera[2, 3] = self.camera_offset_z
        ###########################################################################################
    ...
    def camera_to_image_coordinates(self, camera_x_right, camera_y_down, camera_z_forward):
        ...
        if self.enable_distortion:
            ...
            r = np.sqrt(img_proj_x**2 + img_proj_y**2)
            img_proj_x = (1 + self.distortion_k1 * r**2 + self.distortion_k2 * r**4 + self.distortion_k3 * r**6) * img_proj_x + \
                         2 * self.distortion_p1 * img_proj_x * img_proj_y + \
                         self.distortion_p2 * (r**2 + 2 * img_proj_x**2)
            img_proj_y = (1 + self.distortion_k1 * r**2 + self.distortion_k2 * r**4 + self.distortion_k3 * r**6) * img_proj_y + \
                         self.distortion_p1 * (r**2 + 2 * img_proj_y**2) + \
                         2 * self.distortion_p2 * img_proj_x * img_proj_y
            ###########################################################################################
            # 수치 안정성 체크
            if not (math.isfinite(img_proj_x) and math.isfinite(img_proj_y)):
                return None, None, None
    ...
    def render_buildings(self, image, commands):
        ...
        for building in self.buildings.objects:
            ...
            for point in building.footprint:
                ...
                # 바닥 점 변환
                u0, v0, d0 = self.camera_to_image_coordinates(*self.world_to_camera_frame(point.x, point.y, 0.0))
                # 지붕 점 변환
                u1, v1, d1 = self.camera_to_image_coordinates(*self.world_to_camera_frame(point.x, point.y, height_val))
                ###########################################################################################
                # 바닥 점 처리
    ...
    def render_lanes(self, image):
        ...
        for lane in self.lanes.lanes:
            ...
            for point in lane.lane_lines:
                ...
                u, v, depth = self.camera_to_image_coordinates(*self.world_to_camera_frame(point.x, point.y, 0.03))
        ...
        for guardrail in self.guardrails.objects:
            ...
            def proj(world_pts):
                ...
                for (world_point_x, world_point_y, world_point_z) in world_pts:
                    u, v, d = self.camera_to_image_coordinates(*self.world_to_camera_frame(world_point_x, world_point_y, world_point_z))
                    if u is None or d <= 0:
                        pts.append(None)
                    else:
                        pts.append((u, v))
    ...
    def render_traffic_signs(self, image, commands):
        ...
        for sign in getattr(self.traffic_signs, 'signs', []):
            ...
            u, v, depth = self.camera_to_image_coordinates(*self.world_to_camera_frame(sign.pose.position.x, sign.pose.position.y, sign.pose.position.z))   
            ###########################################################################################
    ...
    def render_traffic_lights(self, image, commands):
        ...
        for light in self.traffic_lights.lights:
            ...
            u, v, depth = self.camera_to_image_coordinates(*self.world_to_camera_frame(light.pose.position.x, light.pose.position.y, light.pose.position.z))
            ##########################################################################################
