...
class RadarEmu:
    ...
    def InRadarFov(self, x_radar: float, y_radar: float) -> bool:
        ...
        # 실습: FOV 내에 있는지 확인
        b_is_front = x_radar > 0  # TODO: 전방에 포인트가 있는지 검사
        b_is_fov = abs(angle_deg) <= (self.fov_deg / 2)  # TODO: FOV 내에 포인트가 있는지 검사, Hint: self.fov_deg 사용
    ...
    def CalculatePower(self, range_m: float, rcs: float, object_type: str) -> float:
        ...
        self.radar_constant = (self.transmit_power_watts * self.transmit_gain_linear * self.receive_gain_linear * (self.wavelength ** 2)) / ((4 * math.pi) ** 3 * self.system_losses_linear)
        received_power_watts = self.radar_constant * effective_rcs / (range_safe**4)
        
        # 실습: 와트를 dBm으로 변환
        if received_power_watts <= 0:
            return -200.0  # 매우 낮은 값 (사실상 감지 불가능)
        
        received_power_dbm = 10.0 * math.log10(received_power_watts * 1000)
    ...
    def ApplyNoise(self, x_radar: float, y_radar: float) -> Tuple[float, float]:
        ...
        # 실습: 노이즈 적용
        range_noisy = max(0.0, range_true + range_noise)  # 실습: 거리 노이즈 적용 (양수 보장)
    ...
    def GenerateClutterPoints(self) -> List[Tuple[float, float, float, float, float, float]]:
        ...
        for _ in range(num_clutter):
            # 실습: FOV 내 랜덤 좌표 생성
            clutter_range = self.rng.uniform(self.clutter_range_min, self.clutter_range_max)  # Hint: self.clutter_range_min, self.clutter_range_max 사용
