        ...
        with open(full, newline='') as f:
            ...
            for row in reader:
                try:
                    ...
                    # ====== TODO: .csv 파싱 ====== #
                    # ============================ #
                    vehicle_id = int(row.get('id', 0)) # Vehicle ID
                    seq = int(row.get('seq', 0)) # Waypoint order, default = 0
                    x = float(row.get('x', 0.0)) # X coordinate
                    y = float(row.get('y', 0.0)) # Y coordinate
                    speed_mps = float(row.get('speed_mps', 0.0)) # Speed
                    # ============================ #
                    # ============================ #
                    # ============================ #
                except Exception:
