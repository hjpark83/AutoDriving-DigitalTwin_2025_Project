...
class IMUEmu:
    ...
    def _apply_noise(self, msg: Imu) -> Imu:
        ...
        angular_velocity_x_noisy = msg.angular_velocity.x + avx_bias
        angular_velocity_y_noisy = msg.angular_velocity.y + avy_bias
        angular_velocity_z_noisy = msg.angular_velocity.z + avz_bias
        
        # 선형가속도에 바이어스와 가우시안 노이즈 추가
        linear_acceleration_x_noisy = msg.linear_acceleration.x + lax_bias
        linear_acceleration_y_noisy = msg.linear_acceleration.y + lay_bias
        linear_acceleration_z_noisy = msg.linear_acceleration.z + laz_bias
        #############################################################
    ...
    def _update_biases(self) -> None:
        ...
        self.gyro_bias = self.gyro_bias + self.rng.normal(0.0, math.pow(self.gyro_bias_rw_std, 2), size=3)
        
        # 가속도계 바이어스 업데이트: b_accel = b_accel + N(0, σ_accel^2 * dt)
        self.accel_bias = self.accel_bias + self.rng.normal(0.0, math.pow(self.accel_bias_rw_std, 2), size=3)
    ...
    def _set_covariances(self, msg: Imu) -> None:
        ...
        oc[0] = math.pow(self.orientation_noise_std, 2)
        oc[4] = math.pow(self.orientation_noise_std, 2)
        oc[8] = math.pow(self.orientation_noise_std, 2)

        # 각속도 공분산 행렬 (3x3): [σ_wx², 0, 0; 0, σ_wy², 0; 0, 0, σ_wz²]
        avc[0] = math.pow(self.angular_velocity_noise_std, 2)
        avc[4] = math.pow(self.angular_velocity_noise_std, 2)
        avc[8] = math.pow(self.angular_velocity_noise_std, 2)

        # 선형가속도 공분산 행렬 (3x3): [σ_ax², 0, 0; 0, σ_ay², 0; 0, 0, σ_az²]
        lac[0] = math.pow(self.linear_acceleration_noise_std, 2)
        lac[4] = math.pow(self.linear_acceleration_noise_std, 2)
        lac[8] = math.pow(self.linear_acceleration_noise_std, 2)
        #############################################################
