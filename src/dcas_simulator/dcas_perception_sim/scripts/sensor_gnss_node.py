...
class GNSSEmu:
    ...
    def meridional_radius(self, ref_latitude_deg: float) -> float:
        ...
        num = np.power(a * b, 2)
        den = np.power(np.power(a * np.cos(lat), 2) + np.power(b * np.sin(lat), 2), 1.5)
    ...
    def normal_radius(self, ref_latitude_deg: float) -> float:
        ...
        # 여기서 φ는 위도, a는 장반축, b는 단반축
        num = np.power(a, 2)
        den = np.sqrt(np.power(a * np.cos(lat), 2) + np.power(b * np.sin(lat), 2))
    ...
    def enu_to_wgs84(self, enu: np.ndarray, ref_llh: np.ndarray) -> np.ndarray:
        ...
        # TODO: 최종 WGS84 좌표 계산
        llh[0] = ref_llh[0] + delta_lat_deg # 위도 (Latitude)
        llh[1] = ref_llh[1] + delta_lon_deg # 경도 (Longitude)
        llh[2] = ref_llh[2] + enu[2] # 고도 (Height)
    ...
    def _apply_noise(self, msg: NavSatFix, msg_odom: Odometry, shadow_pos_std: float = 0.0) -> None:
        ...
        n_alt_m = self.rng.normal(0.0, self.pos_noise_std)   # [m], self.rng.normal() 사용
        ...
        if shadow_pos_std > 0.0:
            sh_lat_m = self.rng.normal(0.0, shadow_pos_std) # self.rng.normal() 사용
            sh_lon_m = self.rng.normal(0.0, shadow_pos_std) # self.rng.normal() 사용
            # TODO: 노이즈를 도 단위로 변환하여 적용
        ...
        # TODO: 음영 지역에서 추가 위치 노이즈 적용
        if shadow_pos_std > 0.0:
            msg_odom.pose.pose.position.x += self.rng.normal(0.0, shadow_pos_std) # self.rng.normal() 사용
            msg_odom.pose.pose.position.y += self.rng.normal(0.0, shadow_pos_std) # self.rng.normal() 사용
        ...
        msg_odom.twist.twist.angular.x += self.rng.normal(0.0, self.vel_noise_std) # self.rng.normal() 사용
        msg_odom.twist.twist.angular.y += self.rng.normal(0.0, self.vel_noise_std) # self.rng.normal() 사용
        msg_odom.twist.twist.angular.z += self.rng.normal(0.0, self.vel_noise_std) # self.rng.normal() 사용
